...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
    ...
    try {
        ...
        $getImageQuery = $koneksi->prepare("SELECT image FROM dokumentasi_event WHERE id = ?");
        $getImageQuery->execute([$id]);
        $imagePath = $getImageQuery->fetchColumn();

        // Menghapus file gambar dari folder
        if ($imagePath && file_exists('../../upload/dokumentasi-event/' . $imagePath)) {
            // Menghapus file gambar dari folder
            unlink('../../upload/dokumentasi-event/' . $imagePath);
        } else {
        ...
        $query = "DELETE FROM dokumentasi_event WHERE id = ?";
